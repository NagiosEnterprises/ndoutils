...
int ndomod_write_object_config(int config_type) {
	...
	for (temp_command = command_list; temp_command != NULL; temp_command = temp_command->next) {
		...
		ndomod_broker_data_serialize(&dbuf, 
			...
			ARRAY_SIZE(command_definition), TRUE);
	...
	for (temp_timeperiod = timeperiod_list; temp_timeperiod != NULL; temp_timeperiod = temp_timeperiod->next) {
		...
		ndomod_broker_data_serialize(&dbuf, 
			...
			ARRAY_SIZE(timeperiod_definition), FALSE);
	...
	for (temp_contact = contact_list; temp_contact != NULL; temp_contact = temp_contact->next) {
		...
		struct ndo_broker_data contact_definition[] = {
			...
			NDODATA_INTEGER(NDO_DATA_SERVICENOTIFICATIONSENABLED, service_notifications_enabled),
			NDODATA_INTEGER(NDO_DATA_HOSTNOTIFICATIONSENABLED, host_notifications_enabled),
			NDODATA_INTEGER(NDO_DATA_CANSUBMITCOMMANDS, can_submit_commands),
			...
			NDODATA_INTEGER(NDO_DATA_NOTIFYSERVICEDOWNTIME, notify_on_service_downtime),
			...
			NDODATA_INTEGER(NDO_DATA_NOTIFYHOSTDOWNTIME, notify_on_host_downtime),
			NDODATA_INTEGER(NDO_DATA_MINIMUMIMPORTANCE, temp_contact->minimum_value),
#else
			NDODATA_INTEGER(NDO_DATA_NOTIFYSERVICEUNKNOWN, temp_contact->notify_on_service_unknown),
			NDODATA_INTEGER(NDO_DATA_NOTIFYSERVICEWARNING, temp_contact->notify_on_service_warning),
			NDODATA_INTEGER(NDO_DATA_NOTIFYSERVICECRITICAL, temp_contact->notify_on_service_critical),
			NDODATA_INTEGER(NDO_DATA_NOTIFYSERVICERECOVERY, temp_contact->notify_on_service_recovery),
			NDODATA_INTEGER(NDO_DATA_NOTIFYSERVICEFLAPPING, temp_contact->notify_on_service_flapping),
			NDODATA_INTEGER(NDO_DATA_NOTIFYSERVICEDOWNTIME, notify_on_service_downtime),
			NDODATA_INTEGER(NDO_DATA_NOTIFYHOSTDOWN, temp_contact->notify_on_host_down),
			NDODATA_INTEGER(NDO_DATA_NOTIFYHOSTUNREACHABLE, temp_contact->notify_on_host_unreachable),
			NDODATA_INTEGER(NDO_DATA_NOTIFYHOSTRECOVERY, temp_contact->notify_on_host_recovery),
			NDODATA_INTEGER(NDO_DATA_NOTIFYHOSTFLAPPING, temp_contact->notify_on_host_flapping),
			NDODATA_INTEGER(NDO_DATA_NOTIFYHOSTDOWNTIME, notify_on_host_downtime),
#endif
		};
		...
		ndomod_broker_data_serialize(&dbuf, 
			...
			ARRAY_SIZE(contact_definition), FALSE);
	...
	for (temp_contactgroup = contactgroup_list; temp_contactgroup != NULL; temp_contactgroup = temp_contactgroup->next) {
		...
		ndomod_broker_data_serialize(&dbuf, 
			...
			ARRAY_SIZE(contactgroup_definition), FALSE);
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		struct ndo_broker_data host_definition[] = {
			...
			NDODATA_FLOATING_POINT(NDO_DATA_HOSTRETRYINTERVAL, (double)retry_interval),
			NDODATA_INTEGER(NDO_DATA_HOSTMAXCHECKATTEMPTS, temp_host->max_attempts),
			NDODATA_FLOATING_POINT(NDO_DATA_FIRSTNOTIFICATIONDELAY, first_notification_delay),
...
#else
			NDODATA_INTEGER(NDO_DATA_NOTIFYHOSTDOWN, temp_host->notify_on_down),
			NDODATA_INTEGER(NDO_DATA_NOTIFYHOSTUNREACHABLE, temp_host->notify_on_unreachable),
			NDODATA_INTEGER(NDO_DATA_NOTIFYHOSTRECOVERY, temp_host->notify_on_recovery),
			NDODATA_INTEGER(NDO_DATA_NOTIFYHOSTFLAPPING, temp_host->notify_on_flapping),
#endif

			NDODATA_INTEGER(NDO_DATA_NOTIFYHOSTDOWNTIME, notify_on_host_downtime),
			NDODATA_INTEGER(NDO_DATA_HOSTFLAPDETECTIONENABLED, temp_host->flap_detection_enabled),
			NDODATA_INTEGER(NDO_DATA_FLAPDETECTIONONUP, flap_detection_on_up),
			NDODATA_INTEGER(NDO_DATA_FLAPDETECTIONONDOWN, flap_detection_on_down),
			NDODATA_INTEGER(NDO_DATA_FLAPDETECTIONONUNREACHABLE, flap_detection_on_unreachable),
			NDODATA_FLOATING_POINT(NDO_DATA_LOWHOSTFLAPTHRESHOLD, temp_host->low_flap_threshold),
			NDODATA_FLOATING_POINT(floating_point, temp_host->high_flap_threshold),
...
#else
			NDODATA_INTEGER(NDO_DATA_STALKHOSTONUP, temp_host->stalk_on_up),
			NDODATA_INTEGER(NDO_DATA_STALKHOSTONDOWN, temp_host->stalk_on_down),
			NDODATA_INTEGER(NDO_DATA_STALKHOSTONUNREACHABLE, temp_host->stalk_on_unreachable),
#endif
...
#else
			NDODATA_INTEGER(NDO_DATA_PASSIVEHOSTCHECKSENABLED, temp_host->accept_passive_host_checks),
#endif
...
#else
			NDODATA_INTEGER(NDO_DATA_HOSTFAILUREPREDICTIONENABLED, temp_host->failure_prediction_enabled),
			NDODATA_INTEGER(NDO_DATA_OBSESSOVERHOST, temp_host->obsess_over_host),
#endif
			...
			NDODATA_INTEGER(NDO_DATA_HAVE2DCOORDS, have_2d_coords),
			NDODATA_INTEGER(NDO_DATA_X2D, x_2d),
			NDODATA_INTEGER(NDO_DATA_Y2D, y_2d),
			NDODATA_INTEGER(NDO_DATA_HAVE3DCOORDS, have_3d_coords),
			NDODATA_FLOATING_POINT(NDO_DATA_X3D, y_3d),
			NDODATA_FLOATING_POINT(NDO_DATA_Y3D, y_3d),
			NDODATA_FLOATING_POINT(NDO_DATA_Z3D, z_3d),
			NDODATA_INTEGER(, ),
			NDODATA_INTEGER(, ),
		...
		ndomod_broker_data_serialize(&dbuf, 
			...
			ARRAY_SIZE(host_definition), FALSE);
